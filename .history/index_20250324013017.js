...
function createEmployeeRecord(array) {
  ...
  function createTimeInEvent(employeeRecord, dateTime) {
    // Validate the dateTime input to ensure it's a non-empty string
    if (!dateTime || typeof dateTime !== 'string') {
      throw new Error('dateTime must be a string and cannot be empty');
    }
  
    // Split the date and hour from the dateTime string
    const [date, hour] = dateTime.split(' ');
  
    // Validate that both the date and hour are extracted correctly
    if (!date || !hour) {
    ...
    employeeRecord.timeInEvents.push({
      type: "TimeIn",
      date,
      hour: parseInt(hour, 10) // Ensure the hour is a number
    });
  ...
  function hoursWorkedOnDate(employeeRecord, date) {
    ...
    // Check if timeIn or timeOut events exist for the provided date
    if (!timeInEvent || !timeOutEvent) {
      throw new Error(`Missing timeIn or timeOut event for date: ${date}`);
    }
  
    return (timeOutEvent.hour - timeInEvent.hour) / 100; // Assuming hourly format is "HHMM"
  }
